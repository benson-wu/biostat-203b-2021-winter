...
local_path <- "/Users/bensonwu/Documents/UCLA/2020-2021/Winter 2021"

#Generate path for mimic data
if (str_detect(os, "Linux")) {
  working_path <- "/home/buwenson/biostat-203b-2021-winter/hw3/"
} else if (str_detect(os, "macOS")) {
  working_path <- str_c(local_path, "/BIOSTAT_203B/biostat-203b-2021-winter")
}
```
...
We can use `left_join` to combine the data sets
```{r}
# TODO
icustays_tble %>% 
  ...
  #merge labevents and chartevents tables
  left_join(labevents_tble, by = c("subject_id", "hadm_id"))  %>%
  left_join(chartevents_tble, by = c("subject_id", "hadm_id", "stay_id")) %>%
  # #Display SQL query
  show_query() %>%
  # #Store query in tibble
  as_tibble() %>%
  collect() %>%
  #Create indicatoer for 30-day mortality
  mutate(days_before_death = as.numeric((ymd_hms(deathtime)-ymd_hms(admittime)),
         units="days")) %>%
  mutate(died_within_30_days = ifelse((days_before_death<=30 & 
                                         !is.na(deathtime)), 1, 0)) %>%
  #Save final tibble to icu_cohort.rds in the shiny folder
  saveRDS(str_c(working_path,"/mimiciv_shiny/icu_cohort.rds")) %>%
    print(width = Inf)

#Double check that there are no repeated subject_id values
icu_cohort<-read_rds(str_c(working_path,"/mimiciv_shiny/icu_cohort.rds"))
n_occur <- data.frame(table(icu_cohort$subject_id))
...
I created a shiny app that explores the ICU cohort dataset. Just a few things to note are (1) you can stratify the summary statistics and graphs by 30 day mortality status and (2) you will need the `generate_vals.R` script in the `mimiciv_shiny` folder in order to make sure that all the objects and data are called correctly in the `app.R` file. 
