...
df_admissions$admission_location[df_admissions$admission_location == "INFORMATION NOT AVAILABLE"] <- NA
...
Unique subjects were examined to avoid having additional count of males or females. There are more females than males in this dataset. 
...
```{r}
system(str_c("zcat < ", shQuote(str_c(mimic_path, "/hosp/labevents.csv.gz")), " | head"), 
       intern = TRUE)
```
...
#Remove the lab events dataframe to clear memory
remove("df_labevents")
...
```{r}
df_d_labitems <- fread(str_c(mimic_path, "/hosp/d_labitems.csv.gz"), nThread = 4)
```
...
df_labevents_subset <- subset(df_labevents, itemid %in% c("50912", "50971", 
  ...
  mutate(charttime = ymd_hms(charttime)) %>% as.tibble() %>%
  print(width = Inf)
```

However, if we weren't provided with the item IDs we can use this method to search for the item IDs based off of the key words.
```{r}
...
Then filter for rows in `labevents_icustays.csv.gz` that contain only these items. The result is the same as the short cut method. 
```{r}
...
Or we can also do it the way that it was done in lecture to streamline it more.
```{r}
...
Trial 1: Cannot read this in even on my local machine. 
```{r}
...
cd /Users/bensonwu/Documents/UCLA/2020-2021/Winter\ 2021/BIOSTAT_203B/mimic-iv/core/
zcat < chartevents.csv.gz | awk -F, '{OFS = ","} {if ($6 == 220045 || $6 == 220052 || $6 == 220050 || $6 == 220181 || $6 == 220179 || $6 == 223761 || $6 == 220277 || $6 == 220210) print $1, $2, $3, $4, $6, $8}' | gzip > chartevents_filtered_itemid.csv.gz
```
...
Create a tibble that contains the item IDs and item names 
```{r}
chartevents_itemid <- tibble(itemid = c(220045, 220052, 220050, 220181, 220179, 223761, 220277, 220210)
, labitem = c("heart_rate", "arterial_BP_mean", "arterial_BP_sys", "noninvasive_bp", "noninvasive_bp_sys", "temperature_far", "SpO2", "respiratory_rate")) %>%
  print(width=inf)
```
...
df_icu_uniq <- df_icu %>% 
  ...
  #Ungroup the tibble
  ungroup() %>%
  #keep all the rows in the ICU stay tibble and then for each row match df_admission by subject_id and hadm_id
...
df_icu_uniq <- df_labevents_subset %>%
  ...
  #only keep the first charttime for each item
     #Sort according to charttime within each group and take the top 1 in each sorted group
...
```{r}
rm("df_admissions", "df_admissions_count", "df_labevents", "df_labevents_subset")
```
...
df_icu_uniq_final <- 
  ...
  #Turn itemid into columns and assign the values to the variables
  spread(key = itemid, value = valuenum) %>% 
  right_join(select(df_icu_uniq,-one_of("stay_id")), by = c("subject_id", "hadm_id")) %>%
  ungroup() %>%
  # #More informative column names from Question 6
  rename_at(vars(as.character(chartevents_itemid$itemid)), 
                 ~chartevents_itemid$labitem) %>%
  #create a variable that represents if a patient died within 30 days of admission
  mutate(days_before_death = as.numeric((ymd_hms(deathtime)-ymd_hms(admittime)), units="days")) %>%
